  ...
  return (
    <nav className="w-full px-6 py-4 flex items-center justify-between border-b border-gray-800/50 bg-gray-950/50 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-2.5">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-green-500/20">
          PB
        </div>
        <span className="text-lg font-bold text-white tracking-tight">
          Private<span className="text-green-400">Bridge</span>
        </span>
      </div>

      {/* Nav Links */}
      <div className="hidden sm:flex items-center gap-6">
        <span className="text-sm font-medium text-green-400 cursor-default">Swap</span>
      </div>
        ...
        {isConnected && address ? (
          ...
          <>
            <button
              ...
              className="px-5 py-2.5 bg-green-500 hover:bg-green-600 text-white font-semibold text-sm rounded-xl shadow-lg shadow-green-500/25 transition-all duration-200 disabled:opacity-50 disabled:cursor-wait flex items-center gap-2"
            >
              {isConnecting ? (
                <>
                ...
                <div className="p-2 space-y-1">
                  {connectors.map((connector) => (
                    <button
                      ...
                      className="w-full px-4 py-3 text-left text-sm text-gray-200 hover:bg-green-500/10 hover:text-green-400 rounded-lg transition-colors flex items-center gap-3"
                    >
